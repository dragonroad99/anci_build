...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^ancient_") ; then
        ANCIENT_BUILD=$(echo -n $1 | sed -e 's/^ancient_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $ANCIENT_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        ANCIENT_BUILD=
    fi
    export ANCIENT_BUILD
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        cd $T > /dev/null
        vendor/ancient/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/ancient/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
